...
export function ClassScheduler({
  ...
  const isPreviousWeekDisabled =
    format(weekStart, "yyyy-MM-dd") === format(today, "yyyy-MM-dd");
  ...
  React.useEffect(() => {
    if (onSelectionChange) {
      const selectedClasses = Object.entries(selectedSlots).map(
        ([key, type]) => {
        ...
        <div className="flex items-center gap-1 border-[1px] border-[#353259] rounded-md">
          <Button
            ...
            className={`hover:bg-[#4F518C] ${
              isPreviousWeekDisabled ? "opacity-50 cursor-not-allowed" : ""
            }`}
          >
            ...
            >
              <div>
                ...
                <div className="text-sm opacity-75">
                  {classPrices.individual.price}{" "}
                  {classPrices.individual.currency}
                </div>
              </div>
            </Button>
